...
export function PlanCard({ plan, ctaBase = '/unidades', ctaLabel }: PlanCardProps) {
  ...
  if (plan.featured) {
    return (
      <article className={cn(
        ...
        <div className="absolute -right-10 top-6 w-36 rotate-45 bg-lf-volt py-1 text-center text-[10px] font-black uppercase tracking-widest text-lf-black">
          {plan.badge}
        </div>
        ...
        <div className="p-6 flex flex-col flex-1">
          ...
          {plan.firstPayment && (
            <div className="mt-5 border-l-2 border-lf-volt bg-lf-graphite py-3 pl-4">
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-lf-volt">
                {plan.firstPayment.label}
              </p>
              <p className="mt-1 text-3xl font-black text-lf-text">{plan.firstPayment.value}</p>
            </div>
          )}

          <p className="mt-5 flex flex-wrap items-end gap-x-2 gap-y-1 text-lf-text">
          ...
          <ul className="mt-6 space-y-2.5 text-sm text-lf-muted">
  ...
  return (
    <article className={cn(
      ...
      'transition duration-300 hover:-translate-y-1 hover:border-lf-volt/60 hover:shadow-sm',
      'min-h-[500px]',
    )}>
      <div className="p-6 flex flex-col flex-1">
        ...
        <h3 className="mt-4 text-xl font-black text-gray-900">{plan.name}</h3>
        <p className="mt-2 text-sm leading-relaxed text-gray-500">{plan.description}</p>

        <p className="mt-5 flex flex-wrap items-end gap-x-2 gap-y-1 text-gray-900">
          <strong className="text-4xl font-black leading-none whitespace-nowrap">{plan.price}</strong>
