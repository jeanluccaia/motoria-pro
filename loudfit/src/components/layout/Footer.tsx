...
import { FooterExpansionCta } from './FooterExpansionCta'
...
const navGroups: { title: string; items: FooterLink[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Perguntas frequentes', href: '/franquias#faq' },
    ],
  },
  {
    title: 'Legal',
...
export function Footer() {
  return (
    <footer className="relative bg-lf-black border-t border-lf-line">
      ...
      <div className="mx-auto w-full max-w-[1360px] px-5 py-10 sm:px-8 md:py-14 lg:px-12">
        ...
        <div className="grid gap-10 lg:grid-cols-[1.15fr_2fr] lg:gap-16">
          ...
          <nav aria-label="Rodapé" className="grid grid-cols-2 gap-6 sm:grid-cols-4">
            {navGroups.map((group) => (
              <div key={group.title}>
                <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.22em] text-lf-volt">
                  {group.title}
                </p>
                <ul className="flex flex-col gap-2.5">
                  {group.items.map((item) => (
                    <li key={item.href + item.label}>
                      <Link
                        href={item.href}
                        className="text-[13.5px] leading-[1.4] text-lf-text/80 transition-colors hover:text-lf-text focus-visible:text-lf-text focus-visible:outline-none"
                      >
                        {item.label}
                      </Link>
                    </li>
        ...
        {/* Contato + institucional em linha compacta */}
        <div className="mt-10 grid gap-6 border-t border-lf-line pt-8 md:grid-cols-3 md:gap-6">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-lf-muted/70">
              Fale com a Loud Fit
            </p>
            <a
              ...
              className="mt-2 block text-[14px] leading-[1.5] text-lf-text/85 transition-colors hover:text-lf-volt"
            >
              {contactInfo.email}
            </a>
            <a
              href={`tel:${contactInfo.phone}`}
              className="mt-1 block text-[14px] leading-[1.5] text-lf-text/85 transition-colors hover:text-lf-volt"
            >
          ...
          <div>
            ...
            <p className="mt-2 text-[12.5px] leading-[1.55] text-lf-text/70">
        ...
        <div className="mt-8 flex flex-col gap-2 border-t border-lf-line pt-5 text-[12px] text-lf-muted/60 sm:flex-row sm:items-center sm:justify-between">
