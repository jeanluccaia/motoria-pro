...
export type FounderNeutralVariant = "unpublished" | "revoked";

export function FounderNeutralPage({
  slug,
  name,
  isTest,
  variant = "unpublished",
  ...
  name: string;
  isTest: boolean;
  variant?: FounderNeutralVariant;
}) {
  const isRevoked = variant === "revoked";
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#070707] px-5 text-white">
      {/* Tracking só faz sentido em variantes válidas — para revogados não emitimos eventos. */}
      {isRevoked ? null : <FounderPublicTracking slug={slug} />}
      <section className="w-full max-w-xl rounded-2xl border border-[#C9A84C]/20 bg-[#101010] p-8 text-center">
        ...
        {isRevoked ? (
          <p className="mt-2 text-sm leading-relaxed text-[#A7A7A7]">
            Este convite foi atualizado. Fale com a DGN para consultar sua condição atual.
          </p>
        ) : (
