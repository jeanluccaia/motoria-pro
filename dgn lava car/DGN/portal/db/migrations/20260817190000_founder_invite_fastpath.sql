-- Fast-path Founder: uma única ação `create_invite` que faz o mesmo trabalho
-- de `approve` + `create_page` em uma transação. Reusa exatamente as regras
-- de validação e auditoria da RPC v2 existente.
--
-- Sem colunas novas. Sem função nova. Sem impacto em Founders confirmados
-- (Nº001/002/003 continuam protegidos pela mesma guarda) nem em campanhas
-- descartadas.
...
begin
  ...
  if p_action in ('save','approve') then
    ...
    -- Fast-path: mesma validação de approve, depois cria a página. Só falha se
    -- já houver página ativa (evita duplicidade silenciosa).
    if p_plan_snapshot is null or p_plan_code is null or p_contracting_mode is null then
      raise exception using errcode='22023',message='plano oficial e modalidade obrigatórios';
    end if;
    if p_plan_snapshot->>'planCode' <> p_plan_code
