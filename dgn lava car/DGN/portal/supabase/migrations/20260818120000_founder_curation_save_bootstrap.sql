-- DGN Growth: permitir Curadoria completa (save + comercial) mesmo quando o
-- prospect ainda não tem crm_campaign_members. Antes disso, apenas
-- `create_invite` fazia bootstrap silencioso; qualquer save de curadoria ou
-- edição comercial retornava P0002 e o operador via "recarregue a página".
--
-- Regras que continuam intactas:
--   • Founders 001–003 permanecem protegidos.
--   • Bootstrap NÃO aprova nem cria página nem ocupa Nº004:
--       founder_status  = 'não_avaliado'
--       commercial_stage = 'aguardando_analise'
--       founder_number   = null
--   • Checagem otimista de concorrência continua aplicada quando o registro
--     já existe (updated_at deve bater).
--   • Nenhuma coluna nova; comportamento aditivo.

------------------------------------------------------------------
-- 1) crm_manage_founder_curation_v2: estende bootstrap para `save`.
------------------------------------------------------------------
...
declare
  ...
  v_price numeric; v_commitment integer; v_bootstrapped boolean:=false;
begin
  if p_campaign_id <> 'founders-2026'
    or p_action not in ('save','approve','create_page','revoke','replace','mark_sent','create_invite')
  ...
  if not found then
    if p_action in ('create_invite','save') then
      -- Bootstrap seguro: só cria o vínculo com defaults inertes. Não aprova
      -- Founder, não cria página, não ocupa número. As transições operacionais
      -- continuam sendo feitas pelos ramos específicos abaixo.
      insert into public.crm_campaign_members(campaign_id, customer_id)
        values(p_campaign_id, v_customer)
        returning * into v_member;
      v_bootstrapped := true;
    else
      raise exception using errcode='P0002',message='registro de campanha inexistente';
    end if;
  else
    -- Registro pré-existente: checagem otimista de concorrência.
  ...
  if p_action in ('save','approve') then
    ...
    if p_contracting_mode = 'monthly'
      and nullif(btrim(coalesce(p_vehicle_category,'')),'') is null then
      raise exception using errcode='22023',message='categoria de veículo obrigatória para Mensal';
    end if;
    if v_link.id is not null then
      raise exception using errcode='23505',message='já existe página ativa; use Revogar antes de gerar novamente';
    end if;
    ...
    update public.crm_campaign_members set
      ...
      plan_snapshot=p_plan_snapshot,
      founder_status='selecionado'::public.crm_founder_status,
      commercial_stage=case when commercial_stage::text='aguardando_analise'
        then 'pronto_para_contato'::public.crm_commercial_stage else commercial_stage end
      where id=v_member.id returning * into v_member;
  ...
  if v_changed then
    insert into public.crm_interactions(customer_id,campaign_id,interaction_type,channel,description,metadata,actor)
      values(v_customer,p_campaign_id,
        case when p_action in ('create_page','replace','create_invite') then 'convite_criado'::public.crm_interaction_type
        else 'status_alterado'::public.crm_interaction_type end,
        'portal','Curadoria Founder: '||p_action||case when v_bootstrapped then ' (bootstrap)' else '' end,
        jsonb_build_object('action',p_action,'linkVersion',v_link.version,'bootstrapped',v_bootstrapped),p_actor);
  ...
  return jsonb_build_object('changed',v_changed,'customerId',p_customer_legacy_id,
    'campaign',v_new||jsonb_build_object('updatedAt',v_member.updated_at,'bootstrapped',v_bootstrapped));
end; $$;
...
------------------------------------------------------------------
-- 2) crm_update_customer_commercial_fields: bootstrap silencioso do member
------------------------------------------------------------------
...
begin
  ...
  if not found then
    -- Bootstrap silencioso: primeiro `Salvar campos comerciais` cria o
    -- vínculo com defaults inertes. Não aprova Founder, não ocupa número.
    insert into public.crm_campaign_members(campaign_id, customer_id)
      values('founders-2026', v_customer_id)
      returning * into v_member;
    v_bootstrapped := true;
  else
  ...
  if not v_changed then
    return jsonb_build_object(
      ...
      'commercial', v_next || jsonb_build_object('updatedAt', v_member.updated_at, 'bootstrapped', v_bootstrapped)
    );
  end if;
  ...
  insert into public.crm_audit_logs (
    ...
    'origin=' || coalesce(nullif(btrim(p_origin), ''), 'dgn-growth-admin')
      || case when v_bootstrapped then '; bootstrap=true' else '' end
  );

  return jsonb_build_object(
    'changed', true,
    'customerId', p_customer_legacy_id,
    'commercial', v_next || jsonb_build_object('updatedAt', v_updated.updated_at, 'bootstrapped', v_bootstrapped)
  );
end;
$$;
