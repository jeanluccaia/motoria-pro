-- Fast-path Founder: bootstrap silencioso de crm_campaign_members para a
-- ação `create_invite` quando o cliente ainda não tem registro na
-- campanha. Sem impacto para as outras ações (save/approve/create_page/
-- replace/mark_sent/revoke), que continuam exigindo o registro prévio.
--
-- Também relaxa a checagem de concorrência apenas quando o registro foi
-- recém-inserido pelo bootstrap: se v_member já existia, a validação de
-- p_expected_updated_at continua sendo aplicada normalmente.
--
-- Nada mais muda: proteção Nº001–003, bloqueio de descartado,
-- validação de plano/modalidade/categoria/motivo/preço permanecem iguais.
...
begin
  ...
  if not found then
    if p_action = 'create_invite' then
      -- Fast-path bootstrap: insere um crm_campaign_members mínimo para
      -- este cliente na campanha. Defaults do schema: founder_status
      -- 'não_avaliado', commercial_stage 'aguardando_analise'.
