...
type LinkRow = { slug?: unknown; enabled?: unknown; is_test?: unknown; expires_at?: unknown; campaign_member_id?: unknown; crm_campaign_members?: unknown };
...
export async function resolveFounderPublicLink(slugValue: string, db?: SupabaseClient) {
  ...
  try {
    ...
    const result = await client.from("crm_founder_public_links").select("slug,enabled,is_test,expires_at,campaign_member_id,crm_campaign_members!inner(campaign_id,founder_number,crm_customers!inner(legacy_id,name))").eq("slug",slug).eq("enabled",true).maybeSingle();
