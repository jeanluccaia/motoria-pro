...
// para uma tela existente. Deve casar exatamente com `primaryNav` em
// components/admin/DgnAdminShell.tsx.
...
test("founder-attention: prioridade alta para click WhatsApp sem resposta", () => {
  ...
  assert.match(result.data?.[0].href ?? "", /^\/admin\/growth\/founders-2026\?customer=/);
});
...
test("curation-opportunities: elegível com score alto aparece com prioridade alta", () => {
  ...
  assert.equal(result.status, "ok");
  assert.equal(result.data?.[0].priority, "alta");
  assert.match(result.data?.[0].href ?? "", /^\/admin\/growth\/curadoria\?customer=/);
});
