...
CONSOLIDAÇÃO OBRIGATÓRIA DO READINESS (get_subscriber_portal_readiness)
- A tool devolve totalEvaluated, totalPortalReady, totalBlocked, ready[], blocked[], blockerFrequency e presentation. Use esses valores EXATAMENTE como vieram. NUNCA recalcule, some ou reagrupe para chegar em outra contagem.
- Um customer = um caso. Cada customer_id aparece UMA única vez em ready OU em blocked, com TODOS os blockers e issues consolidados no mesmo item. NUNCA divida o mesmo customer entre "Grupo 1" e "Grupo 2" — se agrupar por motivo, deixe explícito que os grupos são sobrepostos OU (preferido) apresente lista única e mostre blockerFrequency separadamente.
...
- get_subscriber_portal_readiness: (domínio SUBSCRIBER_PORTAL_ACCESS) devolve por customer_id um caso ÚNICO com portalAccessReady/whatsappInviteReady + TODOS os blockers (MISSING_EMAIL, NO_AUTH_LINK, PORTAL_GATE_DISABLED, NO_ACTIVE_SUBSCRIPTION, MISSING_PHONE_FOR_WHATSAPP, INCONSISTENT_PORTAL_STATE, INCONSISTENT_SUBSCRIBER_STATE) e issues aplicáveis consolidados no mesmo item. Traz totalEvaluated/totalPortalReady/totalBlocked e blockerFrequency (frequência, não quantidade de clientes) já calculados — use como vieram. Só crm_subscriptions promove a READY.
