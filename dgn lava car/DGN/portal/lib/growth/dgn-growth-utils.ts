/**
 * Slice puro de utilidades e tipos do DGN Growth — SEM importar o dataset
 * `dgn-customers.json`. Client components DEVEM importar deste módulo (nunca de
 * `dgn-growth-data.ts`), para que o bundler não puxe os 2.354 clientes do JSON
 * para o bundle público.
 *
 * `dgn-growth-data.ts` re-exporta tudo daqui (para manter compat com o restante
 * do código server) e adiciona apenas o dataset + funções que consomem o
 * dataset. O import daquele módulo carrega side effects (parse do JSON) —
 * mantê-lo fora do grafo client é a barreira de privacidade.
 */
...
export interface DgnCustomer {
  ...
  /**
   * Plano canônico do assinante. Preenchido apenas quando o cliente tem
   * assinatura ativa em `crm_subscriptions` — nesse caso, é o valor bruto de
   * `subscription_plan` (Essential/Smart/Priority/Corporate Care/…). Fonte de
   * verdade para exibir "plano do assinante". Diferente de `recommendedPlan`,
   * que é sugestão de curadoria/lead e nunca substitui o contratado.
   */
  activePlan?: string | null;
  /**
   * Info pré-computada server-side sobre correspondência em
   * KNOWN_SUBSCRIBERS_2026_08_16. Só o rótulo do plano viaja ao client — a
   * base de conhecimento (nomes/telefones/placas) fica no server, nunca
   * chega ao bundle público. `null` = não bateu; `undefined` = ainda não
   * enriquecido (nunca deveria acontecer para payload que veio via
   * growth-page).
   */
  knownSubscriberPlan?: string | null;
  /**
   * Status do assinante conhecido (junto com knownSubscriberPlan). Necessário
   * para regra de elegibilidade Founder distinguir "ativo" (bloqueia aquisição)
   * de "renovacao_pendente" (mensagem diferente ao operador).
   */
  knownSubscriberStatus?: "ativo" | "renovacao_pendente" | null;
...
const CUTOFF_TIMESTAMP = (() => {
  const parsed = parseDgnDateAsUtcTimestamp(DGN_OPERATIONAL_CUTOFF);
  if (parsed === null) throw new Error(`DGN_OPERATIONAL_CUTOFF invalido: ${DGN_OPERATIONAL_CUTOFF}`);
  return parsed;
})();
...
export function hasOperationalLastAttendance(customer: Pick<DgnCustomer, "lastAttendance">) {
  ...
  return lastAttendanceTimestamp !== null && lastAttendanceTimestamp >= CUTOFF_TIMESTAMP;
}
