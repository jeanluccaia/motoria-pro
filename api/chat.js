 ...
 *   1. Rate limit por minuto (Redis distribuído, fallback memória)
 *   2. Rate limit por hora
 *   3. Validação de payload (whitelist de ferramentas, tamanho, tipo)
 *   4. Token de acesso (UUID validado no Redis) OU tier gratuito (2 uses/IP)
 *   5. Débito de crédito atômico antes de chamar a Anthropic
 *   6. Nenhum detalhe interno exposto ao cliente
 */
...
const MAX_INPUT_CHARS  = 1800; // reduzido de 2000 para economizar tokens
...
module.exports = async function handler(req, res) {
  ...
  // ── 4. Verificar autenticação / créditos ─────────────────────────────────────
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`[chat] Erro Anthropic: ${status}`);
      return res.status(500).json({ error: "Erro ao processar. Tente novamente." });
    }

    const data = await response.json();

    // ── 6. Registrar uso gratuito APÓS sucesso (não cobra por erro) ─────────────
    if (isFree) {
    ...
    return res.status(200).json({
      ...
      // Informações de crédito para o frontend atualizar UI
      credits:   creditsRemaining,              // null = free tier
